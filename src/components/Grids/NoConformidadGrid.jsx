...
const NoConformidadGrid = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await axios.get(API_BASE_URL + `/noconformidads?`+ params.toString());
    ...
    const handleDelete = async (id) => {
        if (window.confirm('¿Estás seguro de eliminar este registro?')) {
            try {
                await axios.delete(API_BASE_URL + `/noconformidads/` + id);
    ...
    if (loading) return <div>Cargando NoConformidad...</div>;
    if (error) return <div>Error al cargar NoConformidad: {error.message}</div>;

    return (
        <div className="grid-container">
			<div className="grid-header">
				<h2>NoConformidad</h2>
				<Link to="/NoConformidads/add" className="add-new-button" >Agregar Nuevo NoConformidad + </Link>
			</div>
					...
					<h3>Filtrar</h3>
						...
						<div>
							...
							<input
								...
								id="fechaDeteccion"
								name="fechaDeteccion"
								value={localFilters['fechaDeteccion'] || ''}
								onChange={handleLocalFilterChange}							
							/>
						</div>
						<div>
						...
						<div>
							...
							<input
								...
								id="auditoriaId"
								name="auditoriaId"
								value={localFilters['auditoriaId'] || ''}
								onChange={handleLocalFilterChange}							
							/>
						</div>
						<div>
                ...
                <tbody>
                    {data.map(item => (
                        <tr key={item.id}>
                            ...
                            <td>{item['fechaDeteccion']}</td>
                            <td>{item['responsableId']}</td>
                            <td>{item['estado']}</td>
                            <td>{item['createdAt']}</td>
                            <td>{item['updatedAt']}</td>
                            <td>{item['auditoriaId']}</td>
                            <td>{item['procesoId']}</td>
                            <td className="grid-actions">
                                <Link to={"/NoConformidads/edit/" + item.id }>Editar</Link>
...
export default NoConformidadGrid;