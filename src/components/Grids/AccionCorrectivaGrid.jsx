...
const AccionCorrectivaGrid = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await axios.get(API_BASE_URL + `/accioncorrectivas?`+ params.toString());
    ...
    const handleDelete = async (id) => {
        if (window.confirm('¿Estás seguro de eliminar este registro?')) {
            try {
                await axios.delete(API_BASE_URL + `/accioncorrectivas/` + id);
    ...
    if (loading) return <div>Cargando AccionCorrectiva...</div>;
    if (error) return <div>Error al cargar AccionCorrectiva: {error.message}</div>;

    return (
        <div className="grid-container">
			<div className="grid-header">
				<h2>AccionCorrectiva</h2>
				<Link to="/AccionCorrectivas/add" className="add-new-button" >Agregar Nuevo AccionCorrectiva + </Link>
			</div>
					...
					<h3>Filtrar</h3>
						...
						<div>
							...
							<input
								...
								value={localFilters['fechaImplementacion'] || ''}
								onChange={handleLocalFilterChange}							
							/>
						</div>
						<div>
							...
							<input
								...
								value={localFilters['fechaVerificacion'] || ''}
								onChange={handleLocalFilterChange}							
							/>
						</div>
						<div>
						...
						<div>
							...
							<input
								...
								value={localFilters['noConformidadId'] || ''}
								onChange={handleLocalFilterChange}							
							/>
						</div>
                ...
                <tbody>
                    {data.map(item => (
                        <tr key={item.id}>
                            ...
                            <td>{item['fechaImplementacion']}</td>
                            <td>{item['fechaVerificacion']}</td>
                            <td>{item['estado']}</td>
                            <td>{item['responsableId']}</td>
                            <td>{item['createdAt']}</td>
                            <td>{item['updatedAt']}</td>
                            <td>{item['noConformidadId']}</td>
                            <td className="grid-actions">
                                <Link to={"/AccionCorrectivas/edit/" + item.id }>Editar</Link>
...
export default AccionCorrectivaGrid;