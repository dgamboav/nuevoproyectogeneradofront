...
const ProcesoAuditoriaGrid = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await axios.get(API_BASE_URL + `/procesoauditorias?`+ params.toString());
    ...
    const handleDelete = async (id) => {
        if (window.confirm('¿Estás seguro de eliminar este registro?')) {
            try {
                await axios.delete(API_BASE_URL + `/procesoauditorias/` + id);
    ...
    if (loading) return <div>Cargando ProcesoAuditoria...</div>;
    if (error) return <div>Error al cargar ProcesoAuditoria: {error.message}</div>;

    return (
        <div className="grid-container">
			<div className="grid-header">
				<h2>ProcesoAuditoria</h2>
				<Link to="/ProcesoAuditorias/add" className="add-new-button" >Agregar Nuevo ProcesoAuditoria + </Link>
			</div>
                    ...
                    {data.map(item => (
                        <tr key={item.id}>
                            ...
                            <td className="grid-actions">
                                <Link to={"/ProcesoAuditorias/edit/" + item.id }>Editar</Link>
...
export default ProcesoAuditoriaGrid;