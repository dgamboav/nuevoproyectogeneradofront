...
const ProyectoGrid = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await axios.get(API_BASE_URL + `/proyectos?`+ params.toString());
    ...
    if (error) return <div>Error al cargar Proyecto: {error.message}</div>;

    return (
        <div className="grid-container">
			<div className="grid-header">
				<h2>Proyecto</h2>
				<Link to="/Proyectos/add" className="add-new-button" >Agregar Nuevo Proyecto + </Link>
			</div>
                    ...
                    {data.map(item => (
                        <tr key={item.id}>
                            ...
                            <td className="grid-actions">
                                <Link to={"/Proyectos/edit/" + item.id }>Editar</Link>
